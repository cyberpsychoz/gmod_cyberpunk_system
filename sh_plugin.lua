...
local CRITICAL_INJURIES_TABLES = {
	head = {
		[1] = {name = "Сотрясение мозга", Apply = function(target, client) target:SetAttribute("strenght", target:GetAttribute("strenght") - 10) end}, -- уменьшаем силу (переделать на интеллект) на 10
		[2] = {name = "Потеря сознания", Apply = function(target, client) end},
	},
	torso = {
		[1] = {name = "Внутреннее кровотечение", Apply = function(target, client) end},
		[2] = {name = "Коллапс легкого", Apply = function(target, client) end},
	},
}
...
ix.command.Add("Heal", { -- добавляем команду /heal для лечения персонажей
	description = "Лечит одну из частей тела персонажа.",
	adminOnly = true,
	arguments = {
		ix.type.character,
			...
			if (character) then -- если персонаж существует
				if (part) then -- если указана часть тела
					...
					if (BODY_PARTS[part]) then -- если такая часть тела есть в таблице
						...
						return string.format("Вы исцелили %s's %s.", target:GetName(), BODY_PARTS[part].name) -- возвращаем сообщение об успехе
					else -- если такой части тела нет в таблице
						return "Неправильная часть тела." -- возвращаем сообщение об ошибке
					end
					...
					return string.format("Вы полностью исцелили %s.", target:GetName()) -- возвращаем сообщение об успехе
				end
			else -- если персонажа не существует
				return "Персонажа не существует." -- возвращаем сообщение об ошибке
			end
...
ix.command.Add("Injure", { -- добавляем команду /injure для нанесения урона персонажам
	description = "Атакует определенную часть тела персонажа.",
	adminOnly = true,
	arguments = {
		ix.type.character,
		bit.bor(ix.type.string, ix.type.optional),
					...
					if (BODY_PARTS[part]) then -- если такая часть тела есть в таблице
						...
						if (hp <= 0) then -- если здоровье части тела стало нулевым или меньше
							...
							if (injury) then -- если есть критическое повреждение
								...
								return string.format("У вас травмированы %s's %s с критическими повреждениями: %s.", target:GetName(), BODY_PARTS[part].name, injury.name) -- возвращаем сообщение об успехе с описанием критического повреждения
							else -- если нет критического повреждения
								return string.format("У вас повреждение %s's %s.", target:GetName(), BODY_PARTS[part].name) -- возвращаем сообщение об успехе без описания критического повреждения
							end
							...
							return string.format("Вы получили травму %s's %s.", target:GetName(), BODY_PARTS[part].name) -- возвращаем сообщение об успехе
						end
					else -- если такой части тела нет в таблице
						return "Неправильная часть тела." -- возвращаем сообщение об ошибке
					end
				else -- если часть тела не указана
					return "Вам нужно указать часть тела." -- возвращаем сообщение об ошибке
				end
			else -- если персонажа не существует
				return "Персонажа не существует" -- возвращаем сообщение об ошибке
			end
